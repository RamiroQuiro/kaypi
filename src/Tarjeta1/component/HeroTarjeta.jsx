...
export default function HeroTarjeta() {
  const  heroTarget  = useStoraStyle(
    (state) => (state.styles.heroTarget),shallow);
  const styles = {
  ...
  return (
    <div id="heroTarget" className=" absolute top-0 left-0 w-full h-full   ">
      {/* <svg
        className={` w-full drop-shadow-lg h-auto  fill-black `}
        version="1.2"
      ...
      <div
        ...
        className="absolute top-0 mix-blend-screen clip-path-url left-0 w-full h-full bg-gradient-to-t  from-primary-100 to-primary-500 z-10 "
      ></div>
    </div>
  );
}
